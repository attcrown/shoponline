<template>
    <div class="fontsPublic d-flex justify-space-around align-center pt-2"
        ...
        <div class="d-flex align-center justify-space-around" v-if="$store.state?.uid">
            ...
            <v-btn text color="white" style="font-size: 17px;" @click="$router.push('/PrivateShop/basket')">
                ตะกร้าสินค้า
        ...
        <div class="d-flex align-center">
            <div v-if="$store.state?.uid" class="d-flex">
                <v-menu offset-y v-model="menuMessage">
                    <template v-slot:activator="{ attrs, on }">
                        <div v-bind="attrs" v-on="on">
                            <v-badge :color="alert_message > 0 ? 'error' : ''" :content="alert_message" class="me-3"
                                overlap>
                ...
                <v-menu offset-y v-model="menuLight">
                    <template v-slot:activator="{ attrs, on }">
                        <div v-bind="attrs" v-on="on">
                            <v-badge :color="alert_light > 0 ? 'error' : ''" :content="alert_light" class="me-6"
                                overlap>
